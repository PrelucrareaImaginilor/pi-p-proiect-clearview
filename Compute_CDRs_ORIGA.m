...
if HasTrueLabels
    validA = ~isnan(CDR_area) & ~isnan(TrueLabel);
    if any(validA)
    ...
    validV = ~isnan(CDR_vert) & ~isnan(TrueLabel);
    if any(validV)
    ...
    validH = ~isnan(CDR_horiz) & ~isnan(TrueLabel);
    if any(validH)
...
% prag scor combinat
if HasTrueLabels
    validS = ~isnan(CombinedScore) & ~isnan(TrueLabel);
    if any(validS)
...
MaskNum = double(MaskExists);
TrueLabelnum = double(TrueLabel);

outT = table(idx, Filename, MaskNum, CupPixels, DiscPixels, ...
    CDR_area, CDR_horiz, CDR_vert, TrueLabelnum, PredNum, CombinedScore, ...
    'VariableNames', {'Index','Filename','MaskExists','CupPixels','DiscPixels', ...
